...
class CISChecker:
    ...
    def _detect_os_family(self):
        """Return 'debian', 'rhel', or 'unknown'."""
        if Path("/etc/debian_version").exists():
            return "debian"
        if Path("/etc/redhat-release").exists() or Path("/etc/rocky-release").exists():
            return "rhel"
        rc, out, _ = self.run_cmd("grep -iE 'ID_LIKE|^ID=' /etc/os-release 2>/dev/null")
        out_lower = out.lower()
        if any(x in out_lower for x in ("debian", "ubuntu")):
            return "debian"
        if any(x in out_lower for x in ("rhel", "fedora", "centos", "rocky", "almalinux")):
            return "rhel"
        return "unknown"
    ...
    def check_updates(self):
        ...
        if os_family == "debian":
            if self.pkg_installed("unattended-upgrades"):
                self.add("2.1.1", "Automatic updates (unattended-upgrades)", "PASS",
                         "unattended-upgrades installed")
            else:
                self.add("2.1.1", "Automatic updates (unattended-upgrades)", "FAIL",
            ...
            self.add("2.1.2", "Automatic updates service enabled",
                     "PASS" if rc == 0 else "WARN",
                     "Service/timer enabled" if rc == 0 else
                     "Run: systemctl enable --now unattended-upgrades")

        elif os_family == "rhel":
            # CIS RHEL9/Rocky9 benchmark equivalent: dnf-automatic
            if self.pkg_installed("dnf-automatic"):
                self.add("2.1.1", "Automatic updates (dnf-automatic)", "PASS",
                         "dnf-automatic installed")
            else:
                self.add("2.1.1", "Automatic updates (dnf-automatic)", "WARN",
                         "Not installed (optional on this system)", "dnf install dnf-automatic")
                if self.fix_mode:
                    self.run_cmd("dnf install -y dnf-automatic")
                    print("  Fixed: dnf-automatic installed")
            rc, _, _ = self.run_cmd("systemctl is-enabled dnf-automatic.timer 2>/dev/null")
            if rc == 0:
                self.add("2.1.2", "dnf-automatic.timer enabled", "PASS",
                         "Timer is enabled")
            else:
                self.add("2.1.2", "dnf-automatic.timer enabled", "WARN",
                         "Timer not enabled (optional on this system)",
                         "systemctl enable --now dnf-automatic.timer")
                if self.fix_mode:
                    self.run_cmd("systemctl enable --now dnf-automatic.timer")
                    print("  Fixed: dnf-automatic.timer enabled")
            auto_conf = Path("/etc/dnf/automatic.conf")
            if auto_conf.exists():
                cfg_content = auto_conf.read_text(errors="ignore")
                if "apply_updates = yes" in cfg_content:
                    self.add("2.1.3", "dnf-automatic apply_updates = yes", "PASS",
                             "Updates will be applied automatically")
                else:
                    self.add("2.1.3", "dnf-automatic apply_updates = yes", "WARN",
                             "apply_updates not set to yes",
                             "Set apply_updates = yes in /etc/dnf/automatic.conf")
            else:
                self.add("2.1.3", "dnf-automatic config present", "WARN",
                         "/etc/dnf/automatic.conf not found; install dnf-automatic first")

        else:
            self.add("2.1.1", "Automatic updates", "SKIP",
                     "Could not detect OS family (not Debian or RHEL-based)")
